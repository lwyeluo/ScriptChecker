...
ResultExpr RestrictMprotectFlags() {
  ...
  /*
   * modified by Luo Wu
   * add a secret to arg 6
   */
  return If((prot & ~kAllowedMask) == 0, Allow()).Else(CrashSIGSYS());

//  const Arg<int> secret(5);
//  return If(((prot & ~kAllowedMask) != 0), CrashSIGSYS())
//          .ElseIf((prot == PROT_NONE), Allow())
//          .ElseIf((secret == 0xabcd), Allow())
